...
- 🛎️ **我的 Skills 更新提示**：从商店安装且远端版本更高的 Skill，会在“我的 Skills”的列表与画廊视图显示 `Update available` 提示
...
- ✏️ **卡片详情快速编辑**：桌面端 card view 的右侧详情标题支持双击进入轻量内联编辑，完整编辑弹窗入口仍保留
- 🔄 **预览更新链路加固**：预览通道只检查 prerelease feed；关闭预览通道后才会回到 stable 检查，且不会自动从较新预览版降级到较旧稳定版
- ⚙️ **Skill 设置与平台顺序恢复**：Skill 相关设置恢复到独立设置页，并支持按默认优先级显示和拖拽调整平台顺序
