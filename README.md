...
- [x] **稳定 / 预览更新通道**：默认只更新稳定版，加入预览版本后才会接收 GitHub prerelease 构建
...
- 🚦 **稳定 / 预览更新通道**：系统设置可加入预览版本；默认只检查稳定版，加入后才会检查 GitHub prerelease 构建
...
### 预览通道修复 v0.5.5-beta.2 (2026-05-03)

- ✏️ **卡片详情快速编辑**：桌面端 card view 的右侧详情区可直接修改选中 Prompt 的标题和当前可见用户提示词，完整编辑弹窗仍保留
- 🔄 **预览更新链路修复**：补发带 prerelease 标识的 `0.5.5-beta.2` 构建，确保预览通道可被客户端和 GitHub Releases 可靠识别
- 🚦 **稳定通道保护**：该构建在 semver 上低于 `0.5.5` stable，稳定通道不会把它作为默认自动更新目标
- 🧹 **发布文档语气整理**：README、多语言 README 与 release workflow 的预览通道说明改为正式发布文档口径
