...
> - **`0.5.5-beta.3`**：该构建继续加固 preview-only 更新链路。关闭预览通道后才会回到 stable 检查；如果当前预览版高于最新 stable，PromptHub 不会自动降级
...
### 预览通道完善 v0.5.5-beta.3 (2026-05-04)

- 🖱️ **卡片详情快速编辑交互收口**：右侧详情标题改为双击直接进入轻量编辑，移除突兀的大按钮，完整编辑弹窗仍保留
- 🔄 **预览更新链路继续加固**：预览通道检查更新时不再把 stable release 当作回退候选；只有关闭预览通道后才会回到 stable 检查
- 🚫 **不自动降级到正式版**：如果当前预览版高于最新 stable，PromptHub 不会自动从预览版降级到旧的正式版
- 🧪 **回归覆盖补强**：补充 beta/stable SemVer 顺序和 prerelease feed 选择回归测试，降低后续发版回归风险
