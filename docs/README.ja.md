...
- 🧩 **ストア Skill 更新検出と競合保護**：インストール時のハッシュとバージョンを保存し、最新のリモート `SKILL.md` と比較。ローカルとリモートが同時に変わった場合は競合として扱います
- 🌍 **完全な `SKILL.md` 翻訳 sidecar**：Skill 翻訳は完全な文書から sidecar 訳文を保存し、全文翻訳と没入型の対照表示に対応します
- 💬 **AI テストと翻訳エラー表示の改善**：モデルテストは「XX モデル テスト成功 / 失敗」を表示し、未設定・タイムアウト・`504` も明確に案内します

**Desktop**

- 🗂️ **データディレクトリ切り替えが本当に再起動で反映**：専用 relaunch IPC で新しい `userData` パスを適用し、現在のディレクトリ再選択時の誤った再起動要求も解消しました
- 🎞️ **動画保存境界の強化**：動画保存はファイルピッカーが直前に返した保存先と対応拡張子だけを受け付けます
- 🔄 **プラットフォーム配備で内部 sidecar を非表示化**：Skill の symlink 配備は canonical `SKILL.md` のみをリンクし、`.prompthub` sidecar を外部プラットフォームに露出しません

**Web / Docs**

- 🌐 **メディアアップロードと表示修正**：Web/Docker で選択したメディアをアップロードし、`local-image://` / `local-video://` も表示できます
- 🔐 **同期私密状態とパスワード修正**：`visibility` のないデスクトップフォルダーは Web で私密扱いされず、セルフホスト Web 設定にはログインパスワード変更フォームも追加されました
- 🌍 **リリース文書の再同期**：README、多言語ドキュメント、CHANGELOG、サイト公開メタデータを再同期し、最終的な `v0.5.5` 説明を実装内容に合わせました
