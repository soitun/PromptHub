...
最新安定版 **v0.5.6**。[Releases](https://github.com/legeling/PromptHub/releases/latest) から直接、または下記の各プラットフォーム別リンクから入手できます。
...
| プレビュー       | [![Preview Prereleases](https://img.shields.io/badge/Preview_Prereleases-8B5CF6?style=for-the-badge&logo=github&logoColor=white)](https://github.com/legeling/PromptHub/releases?q=prerelease%3Atrue) |

> Apple Silicon は `arm64`、Intel Mac は `x64`。Windows のほとんどは `x64` で、Surface Pro X など ARM 機のみ `arm64` を選んでください。
...
> v0.5.6 のスクリーンショットは更新中です。下記はコアな操作を示しています。新画面（Rules、プロジェクト Skill、Quick Add、タグマネージャ）はアプリ上で直接確認してください。
